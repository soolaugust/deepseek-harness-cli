...
dsh 交互式终端组合包。[`cordis.patch.yml`](cordis.patch.yml) 直接叠加在 [`dsh-base`](../base/README.md) 之上：提供编码 persona 和工具模式、禁用 HMR（热模块替换）、将 Code Mode 的 worker 作为核心执行能力挂载、插入智能体预设清单（默认 `standard`），并插入本包的 `cli-startup` 提供方和 `cli-runner` 插件。它不挂载任何 Host、HTTP server、Web runtime 或浏览器插件。
...
cli 组合包挂载了 [`agent-presets`](../../preset/agent-presets/README.md) 清单，其随附预设位于 `config/agent-presets/`，默认预设为 `standard`，因此每个会话都从某个预设组合而成：预设决定模型所见到的工具 schema 与提示词片段。`dsh cli --mode <preset>` 把全新或恢复的会话固定到该预设（省略时默认采用清单自身的默认预设）；恢复会话时，runner 从会话日志重建其记录的预设，而非仅看创建时的头部，因此空白期内切换过预设的会话会以更新的组合运行其后续回合。会话内，`/mode <preset>` 把仍处于空白状态的 agent 重新链接到另一预设，`/mode` 无参数时列出清单——两者都镜像 Web 面的智能体预设切换，即会话开始后拒绝切换。替换掉清单或未挂载任何 `agent-presets` 行的 profile 不组合任何预设，此时 `--mode` 与 `/mode` 均无效果。
