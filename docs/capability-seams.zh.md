...
flowchart LR
  ...
  pkg_agent_model_selection["agent-model-selection"]
  svc_agentModelSelection["ctx.agentModelSelection<br/>Per-Agent model selection reference"]
  ...
  pkg_agent_model_selection --> svc_agentModelSelection
  ...
  svc_agentModelSelection --> pkg_agent_default_model
  svc_agentModelSelection --> pkg_agent_model_selection
  svc_agents --> pkg_acp
...
| `ctx.agentModelSelection` | `core` | [`agent-model-selection`](../packages/core/agent-model-selection) | - | [`agent-model-selection`](../packages/core/agent-model-selection), [`agent-default-model`](../packages/core/agent-default-model) | - | 为每个 Agent 拥有一个进程本地的 ModelSelectionRef，为命令平面与模型路由暴露 install/ref。 |
