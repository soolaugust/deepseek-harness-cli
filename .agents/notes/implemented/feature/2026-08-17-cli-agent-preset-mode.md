...
`dsh cli --mode <id>` and `/mode` surface agent-preset selection on the CLI with the Web's blank-gate and log-resume semantics. The mechanism is opt-in: without an agent-presets row in the profile, `--mode` and `/mode` are inert and `dsh cli` runs exactly as before — so the change ships at zero risk to the current shipped profile. The roster-mount decision is left open: mounting it would activate mode selection by default and is the front-loaded work for the full-parity option.
...
- Loader-smoke (`smoke.e2e.ts`): `dsh cli --mode code` parses and runs on a clean tree with no roster (the flag is inert), proving the flag does not break composition when no presets are mounted.
